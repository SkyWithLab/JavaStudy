...
public class Variable {
	...
	public static void declareVariable() {
		/*
		 * 변수 선언: 값을 저장하기 위한 변수를 메모리 공간에 할당하는 것을 의미
		 * 
		 * [표현식]
		 * 			자료형 변수명;
		 * 
		 * -자료형: 변수의 크기나 모양을 지정하는 부분
			...
			명명 규칙: 
			[1] 카멜케이스: 소문자로 시작하고, 두번째 단어가 붙을 경우 첫 글자를 대문자로 시작한다.
			    -상수: 스네이크케이스 (SNAKE_CASE). 모두 대문자로 작성하고 다른 단어는 _로 구분한다.
			[2] 예약어는 사용할 수 없다. (EX. static void public...)
			[3] 공백 사용 불가
			[4] 특수 문자는 _와 $만 사용가능
			[5] 숫자로 시작할 수 없다.
			[6] 대소문자를 구분, 길이에는 제한이 없다.
		 */ 
		
		//1. 논리형(boolean) : 논리값을 저장. true, false. 1byte.
		
		//2. true 값을 저장하는 isTrue라는 변수 선언 및 초기화
